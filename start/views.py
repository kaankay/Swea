...
from weaeinkauf.masterdata_class import Masterdata
    ...
    if request.user.is_authenticated:
        ...
        if post:
            ...
            if user is not None:
                ...
                # API-Call um Stammdaten abzurufen
                access_token = Masterdata().get_auth_token(username, password)
                if access_token:
                    get_stammdaten = Masterdata().get_installations(access_token)

                    # Speichern von get_stammdaten in der Session
                    request.session['get_stammdaten'] = get_stammdaten
