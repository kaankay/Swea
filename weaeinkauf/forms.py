...
class ServicePreisForm(forms.ModelForm):
    class Meta:
        model = ServicePreis
        fields = ['name','laufzeit','basisjahr','grenzwert','bemerkung','kuendigungsrecht']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control'}),
            'laufzeit': forms.NumberInput(attrs={'class': 'form-control'}),
            'basisjahr': forms.NumberInput(attrs={'class': 'form-control'}),
            'grenzwert': forms.NumberInput(attrs={'class': 'form-control'}),
            'bemerkung': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
            'kuendigungsrecht': forms.TextInput(attrs={'class': 'form-control'}),
        }

class VerguetungForm(forms.ModelForm):
    class Meta:
        model = ServicePreisVerguetung
        fields = ['jahr','indexierung','minPreis','fix','var','bemerkung']
        widgets = {
            'jahr': forms.NumberInput(attrs={'class': 'form-control', 'style': 'width:7ch'}),
            'indexierung': forms.NumberInput(attrs={'class': 'form-control'}),
            'minPreis': forms.NumberInput(attrs={'class': 'form-control'}),
            'fix': forms.NumberInput(attrs={'class': 'form-control', 'style': 'width:12ch'}),
            'var': forms.NumberInput(attrs={'class': 'form-control', 'style': 'width:11ch'}),
            'bemerkung': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
        }
...
VerguetungFormSet = inlineformset_factory(ServicePreis, ServicePreisVerguetung, form=VerguetungForm, extra=1, can_delete=True)
...
VerguetungFormSetEdit = modelformset_factory(ServicePreisVerguetung, form=VerguetungForm, extra=0, can_delete=True)
